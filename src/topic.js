...
import { safeGetJSON, buildTopicTitle } from './utils.js';
import { tgCall, topicCreateInFlight, getAllKeys } from './api.js';

// 为话题建立 thread->user 映射，避免管理员命令时全量 KV 反查
export async function createTopic(from, key, env, userId) {
    const title = buildTopicTitle(from);
    if (!env.SUPERGROUP_ID.toString().startsWith("-100")) throw new Error("SUPERGROUP_ID 必须以 -100 开头");
...
export async function getOrCreateUserTopicRec(from, key, env, userId) {
...
// 更新话题状态
export async function updateThreadStatus(threadId, isClosed, env) {
    try {
